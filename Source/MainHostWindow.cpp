...
public:
    PluginListWindow (MainHostWindow& owner_, AudioPluginFormatManager& pluginFormatManager)
        : DocumentWindow ("Available Plugins", Colours::white,
          ...
          owner (owner_)
    {
		//greg1     const File deadMansPedalFile (getAppProperties().getUserSettings()
		//greg1                         ->getFile().getSiblingFile ("RecentlyCrashedPluginsList"));

   /*     setContentOwned (new PluginListComponent (pluginFormatManager,
                                                  owner.knownPluginList,
                                                  deadMansPedalFile,
//greg1                                                   getAppProperties().getUserSettings(), true), true);*/

        setResizable (true, false);
        setResizeLimits (300, 400, 800, 1500);
        setTopLeftPosition (60, 60);

		//greg1  restoreWindowStateFromString (getAppProperties().getUserSettings()->getValue ("listWindowPos"));
        setVisible (true);
    }

    ~PluginListWindow()
    {
		//greg1  getAppProperties().getUserSettings()->setValue ("listWindowPos", getWindowStateAsString());
                      ...
                      DocumentWindow::allButtons)
{
	...
	//greg1   ScopedPointer<XmlElement> savedAudioState (getAppProperties().getUserSettings()
	//greg1                                        ->getXmlValue ("audioDeviceState"));

	//greg1
//    deviceManager.initialise (256, 256, savedAudioState, true); //migfht be importnt
	...
	//greg1 restoreWindowStateFromString (getAppProperties().getUserSettings()->getValue ("mainWindowPos"));
...
*/
  //  addKeyListener (getCommandManager().getKeyMappings()); //greg1
	...
	//greg1     getCommandManager().setFirstCommandTarget (this);
}
...
void MainHostWindow::changeListenerCallback (ChangeBroadcaster* changed)
{
    if (changed == &knownPluginList)
    {
        ...
        if (savedPluginList != nullptr)
        {
			//greg1     getAppProperties().getUserSettings()->setValue ("pluginList", savedPluginList);
			//greg1 getAppProperties().saveIfNeeded();
        }
    }
...
PopupMenu MainHostWindow::getMenuForIndex (int topLevelMenuIndex, const String& /*menuName*/)
{
    ...
    {
        // "File" menu
   //     menu.addCommandItem (&getCommandManager(), CommandIDs::newFile); //greg1
    //    menu.addCommandItem (&getCommandManager(), CommandIDs::open); //greg1

        RecentlyOpenedFilesList recentFiles;
		//greg1         recentFiles.restoreFromString (getAppProperties().getUserSettings()
		//greg1                             ->getValue ("recentFilterGraphFiles"));

        PopupMenu recentFilesMenu;
        recentFiles.createPopupMenuItems (recentFilesMenu, 100, true, true);
        menu.addSubMenu ("Open recent file", recentFilesMenu);

//        menu.addCommandItem (&getCommandManager(), CommandIDs::save); //greg1
        //menu.addCommandItem (&getCommandManager(), CommandIDs::saveAs);//greg1
        menu.addSeparator();
      //  menu.addCommandItem (&getCommandManager(), StandardApplicationCommandIDs::quit); //greg1
    }
    ...
    else if (topLevelMenuIndex == 2)
    {
        ...
        //greg1 menu.addCommandItem (&getCommandManager(), CommandIDs::showPluginListEditor);
        ...
        menu.addSeparator();
		//greg1 menu.addCommandItem (&getCommandManager(), CommandIDs::showAudioSettings);
		//greg1 menu.addCommandItem (&getCommandManager(), CommandIDs::toggleDoublePrecision);

        menu.addSeparator();
		//greg1     menu.addCommandItem (&getCommandManager(), CommandIDs::aboutBox);
    }
    else if (topLevelMenuIndex == 3)
    {
		//greg1     menu.addCommandItem (&getCommandManager(), CommandIDs::allWindowsForward);
    }
...
void MainHostWindow::menuItemSelected (int menuItemID, int /*topLevelMenuIndex*/)
{
    ...
    else if (menuItemID >= 200 && menuItemID < 210)
    {
		...
		//greg1 getAppProperties().getUserSettings()->setValue ("pluginSortMethod", (int) pluginSortMethod);
...
void MainHostWindow::showAudioSettings()
{
	...
	//greg1  getAppProperties().getUserSettings()->setValue ("audioDeviceState", audioState);
	//greg1 getAppProperties().getUserSettings()->saveIfNeeded();
